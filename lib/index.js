...
try {
  oniguruma = require('oniguruma');
  OnigRegExp = oniguruma.OnigRegExp;
  debug('oniguruma is enabled');
} catch (e) {
  OnigRegExp = onigasm.OnigRegExp;
  loadWASM = onigasm.loadWASM;
  debug('onigasm is enabled');
}
...
async function doInit () {
  ...
  try {
    const file = require.resolve('onigasm/lib/onigasm.wasm');
    debug(`file: ${file}`);
    const wasm = await readFileAsync(file);
    await loadWASM(wasm.buffer);
    debug('onigasm.wasm is loaded');
    return;
  } catch (e) {
    throw new Error('failed to load onigasm.wasm');
  }
}
